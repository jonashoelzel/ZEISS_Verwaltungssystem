...
Das sind die Grundanforderungen, welche der Kunde ZEISS an das Programm hat. Jedoch soll von Anfang an darauf geachtet werden, dass das Programm in verschiedner Hinsicht ausbaufähig bleibt - so soll in Zukunft eine Kommunikation zwischen den Nutzern oder ein Statusbericht der Fachbeiträge möglich sein.
...
|*Herr Lösch*|Betreuer, Interessensvertreter für ZEISS|"überwacht"/kontrolliert in regelmäßigen Abständen Projektfortschritt und sorgt dafür, dass Anforderungen von ZEISS erfüllt werden|sehr hoch|_siehe ZEISS_|_siehe ZEISS_
|[.small]#*Paul, Vanessa, Stephan, Peter, Jean-Pierre, Jonas, Oliver, Nina*#|Projektmitglieder|aktives und eigenständiges Umsetzen der Anfoderungen von ZEISS|extrem hoch|sehr gering, lediglich Unstimmigkeiten zwischen Projektmitgliedern möglich → durch klare Aufgabenverteilung minimiert|positiv
...
|*Mitarbeiter von ZEISS*|Veröffentlichende, Management, sonstige MA|bestimmen indirekt die Anfoderungen an das Projekt|mittel (Anforderungen entspringen dem unerfüllten Bedarf an ein funktionierendes Verwaltungssystem, sind aber fix)|gering, höchstens in der Implementierung|positiv
...
|*Benutzerfreundlichkeit*|sehr hoch|Facheiträge können extrem schnell durchsucht, systematisiert und verändert werden|x
...
|*Zuverlässigkeit*|mittel|Ausfälle/Störungen können durch extrem einfche Wartbarkeit schnell behoben werden|x
