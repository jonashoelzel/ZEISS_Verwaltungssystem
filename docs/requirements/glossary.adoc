= Glossar: ZEISS-Verwaltungssystem
Nina Püschmann s79289@htw-dresden.de
{localdatetime}
include::../_includes/default-attributes.inc.adoc[]
// Platzhalter für weitere Dokumenten-Attribute


== Einführung
In diesem Dokument werden die wesentlichen Begriffe aus dem Anwendungsgebiet (Fachdomäne) der <Projektthema-Systems> definiert. Zur besseren Übersichtlichkeit sind Begriffe, Abkürzungen und Datendefinitionen gesondert aufgeführt.

== Begriffe
[%header]
|===
|Begriff|Definition und Erläuterung|	Synonyme

|Autor|Kann Beiträge veröffentlichen, diese einsehen und bearbeiten|x
|Bereichsleiter | kann Veröffentlichung unterstellter Autoren sehen | x
|Reporter|Kann Reports erstellen, Statistiken einsehen (Top Autoren, Veröffentlichungen je Bereich, ...), Beiträge einsehen und verändern.|x


|===


== Abkürzungen und Akronyme
[%header]
|===
|Abkürzung|	Bedeutung|	Erläuterung|
|UP|Unified Process|Vorgehensmodell für die Softwareentwicklung|
|UI|User Interface|Benutzeroberflächen oder Benutzerschnittstellen, der die Art und Weise beschreibt, mit der ein Anwender mit dem Programm in Kontakt tritt|
|PL|Presentation Layer/Logic|Layer für UI-Code/Logik (Validierung von Benutzereingaben und anzeigen von bestimmten Ergebnissen)|
|BL|Business Layer/Logic|Layer für Business-Code/Logik (Datenvalidierung und -Konvertierung, sowie Code der Softwarefunktionalitäten)|
|IL|Implementation Layer/Logic|Layer für Datenverarbeitung und externe Komponenten (Alternativer Begriff: Busíness Layer) (Lesen/Schreiben von Daten und Verwaltung externer Komponenten)|
|DL|Data Layer/Logic|Layer für Datenverarbeitung und externe Komponenten (Lesen/Schreiben von Daten und Verwaltung externer Komponenten)|
|--|Layer|Sammlung von zusammengehörigen Komponenten, die eine Unabhängigkeit bestimmter Funktionalitäten gewährleistet. Ein Layer trennt verschiedene Logiken|
|--|Logic|Sammlung zusammengehöriger Komponten, die eine bestimmte Aufgaben wie UI, Datenverwaltung, [...] erledigen. Die Logic wird in verschiedene Layer eingeteilt|
|===

== Verzeichnis der Datenstrukturen
[%header]
|===
|Bezeichnung|	Definition |	Format | Gültigkeitsregeln | Aliase
|Anmeldedaten|Zusammensetzung von Benutzername und Passwort.|String|Emailadresse muss @-Zeichen und Punkt enthalten.|Login
|xx|xx|xx|xx|xx|
|===


