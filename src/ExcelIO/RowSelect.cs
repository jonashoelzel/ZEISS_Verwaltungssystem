...
namespace Zeiss.PublicationManager.Data.Excel.IO.Read
{
    ...
    public class RowSelect : ExcelIOBase
    {
        ...
        */
        ...
        protected static Dictionary<string, List<object>> Reader(SheetData sheetData, SharedStringTable sharedStringTable, List<string> columnLetterIDs, int startRowIndex = 1)
        {

            Dictionary<string, List<object>> rowsList = new(columnLetterIDs.Select(x => new KeyValuePair<string, List<object>>(x, new())));
            ...
            foreach (Row row in sheetData.Elements<Row>())
            {
                ...
                if (row?.RowIndex?.Value >= startRowIndex)
                {
                    foreach (Cell cell in row.Elements<Cell>())
                    {
                        ...
                        if (rowsList.ContainsKey(letterID))
                            rowsList[letterID].Add(ReadCell(cell, sharedStringTable));
                    }
